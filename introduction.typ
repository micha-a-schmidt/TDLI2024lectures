...
[$nu$], hel(-1), [$-1/2$], [1], [0], 
[$overline(nu)$], hel(-1), [$-1/2$], [0], [ $(m/E)^2 << 1$], 
[$nu$], hel(1), [$1/2$], [$(m/E)^2 <<1$], [0], 
...
#slide[
 ...
 When SM was formulated, neutrinos were considered massless $arrow.r$ RH neutrinos not needed
 
 $ 
 - cal(L) = overline(L_L) H Y_e e_R + overline(Q_L) H Y_d d_R + overline(Q_L) tilde(H) Y_u u_R + hc
 #h(2cm)
 "with" tilde(H) equiv epsilon H^*
 $

#text(hl1)[Neutrino masses may be introduced similar to charged fermions:  #h(1cm) 
$ -cal(L)_nu = overline(L_L) tilde(H) Y_nu nu_R + hc $]
...
#box(fill:luma(230),width:100%,inset:10pt,radius:5pt, 
[ 
  *nomenclature*
- $nu_R$ usually called _sterile_ neutrinos, since they are gauge singlets
- $nu_L$ usually called _active_ neutrinos, since they participate in electroweak interactions
])
...
$ - overline(L) tilde(H) Y_nu nu_R +hc xarrow(expval(H) = 1/sqrt(2) vec(0,v)) - overline(nu_L) M_nu^D nu_R +hc #h(1cm) "with" M_nu^D = Y_nu v/sqrt(2) $
...
- All terms of the SM conserve lepton number, in particular
 $ 
 - cal(L) = overline(L_L) H Y_e e_R + overline(L_L) tilde(H) Y_nu nu_R + hc
 $
...
The PMNS matrix is parameterised in terms of 3 angles $theta_(i j)$ and 1 Dirac CP phase $delta$. For Majorana neutrinos there are 2 additional Majorana phases $alpha_i$.
